...
public class VRVerifyCodeView: UIView {
    ...
    func initSubviews() {
        // 每个验证码框宽度
        let itemWidth: CGFloat = CGFloat ((ScreenWidth - padding * 2 - spacing * (CGFloat(inputTextNum) - 1))) / CGFloat(inputTextNum)
        for i in 0..<inputTextNum {
            let codeNumView = VRVerifyCodeNumberView(frame: CGRect(x: padding + CGFloat(i) * (spacing + itemWidth), y: 0, width: itemWidth, height: self.frame.height)).isUserInteractionEnabled(false).backgroundColor(.white).cornerRadius(8)
